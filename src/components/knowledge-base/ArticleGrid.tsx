...
type Props = {
    ...
    basePath: "knowledge-base" | "case-studies"; // tell the grid what section it's for
    title?: string;
    id?: string; // for aria-labelledby or in-page anchors
};

export default function ArticleGrid({ articles, basePath, title, id }: Props) {
    const hasItems = articles?.length > 0;

    return (
        <section className="mx-auto max-w-6xl px-4 pb-24" aria-labelledby={id}>
            {title ? (
                <h2 id={id} className="mb-6 text-2xl font-bold tracking-tight text-fg">
                    {title}
                </h2>
            ) : null}
